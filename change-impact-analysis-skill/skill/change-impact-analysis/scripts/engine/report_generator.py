...
class ReportGenerator:
    ...
    def _build_markdown(self, result: Dict[str, Any]) -> str:
        ...
        detection = result.get("change_detection_method", "explicit")
        detection_label = {
            "git": "Git diff",
            "mtime": "Filesystem scan (no Git diff available — heuristic, not exact)",
            "explicit": "Explicit file list",
        ...
        lines: List[str] = [
            ...
            f"**Change Detection:** {detection_label}",
            "",
        ]

        if detection == "mtime":
            lines += [
                "> **Note:** No usable `git diff` was found (this is not a Git "
                "repository, Git is not installed, or there is nothing to diff "
                "against). The changed-file list below was inferred from file "
                "modification times instead. For a precise, deterministic "
                "report, re-run with `--changed-files <files...>`.",
                "",
            ]
